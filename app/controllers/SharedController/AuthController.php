<?php
// ============================================================
// app/controllers/SharedController/AuthController.php
// Procedural controller — no classes (per CONTRIBUTING.md rule).
// Every action is a function named "auth_<action>" so the router
// can dispatch ?c=auth&a=login → auth_login($conn). $conn comes in
// as a parameter from the router; never accessed via $this.
// ============================================================
...
function auth_login($conn) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($errors)) {
            $user = user_get_by_email($conn, $email);
            ...
            if (!$user || !password_verify($password, $user['password_hash'])) {
                ...
                if ($user['role'] === 'seller') {
                    $seller = seller_get_by_user_id($conn, $user['id']);
                    if ($seller) {
...
// ----------------------------------------------------------------
function auth_sellerRegister($conn) {
    $errors = array();
    $old    = array();
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($errors)) {
            ...
            $user_id = user_create($conn, $user_data);
            ...
            if ($user_id) {
                ...
                $seller_id = seller_create($conn, $seller_data);
...
// ----------------------------------------------------------------
function auth_pendingSeller($conn) {
