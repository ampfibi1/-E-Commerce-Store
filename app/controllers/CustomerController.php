...
class CustomerController {
    ...
    public function dashboard() {
        ...
        $all_orders     = order_get_by_customer($this->conn, $uid);
        $recent_orders  = array_slice($all_orders, 0, 5);
        $wishlist_items = wishlist_get($this->conn, $uid);
    ...
    public function profile() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($action === 'update_info') {
                ...
                if (empty($errors)) {
                    user_update($this->conn, $uid, array(
                        'name'  => $name,
                        'email' => $email,
                        'phone' => $phone,
                    ));
            ...
            // ----------------------------------------------------------
                ...
                if (empty($errors)) {
                    user_update_password($this->conn, $uid, password_hash($new_password, PASSWORD_BCRYPT));
                ...
                if (!isset($_FILES['profile_pic']) || $_FILES['profile_pic']['error'] === UPLOAD_ERR_NO_FILE) {
                    ...
                    if ($file['error'] !== UPLOAD_ERR_OK) {
                        ...
                        if (!move_uploaded_file($file['tmp_name'], $dest)) {
                            ...
                            user_update_pic($this->conn, $uid, 'profile_pics/' . $filename);
    ...
    public function addresses() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($action === 'add') {
                ...
                if (empty($errors)) {
                    address_create($this->conn, array(
                        'customer_id'  => $uid,
                        'label'        => $label,
                ...
                if (empty($errors)) {
                    address_update($this->conn, $id, $uid, array(
                        'label'        => $label,
                        'address_line' => $address_line,
                        'city'         => $city,
                        'zip'          => $zip,
                ...
                $id = isset($_POST['id']) ? (int)$_POST['id'] : 0;
                if ($id > 0) {
                    address_delete($this->conn, $id, $uid);
                }
                ...
                $id = isset($_POST['id']) ? (int)$_POST['id'] : 0;
                if ($id > 0) {
                    address_set_default($this->conn, $uid, $id);
                }
        ...
        $addresses = address_get_by_customer($this->conn, $uid);
    ...
    public function products() {
        ...
        $products   = product_get_all($this->conn, $filters);

        $wishlist_ids = array();
        if (isset($_SESSION['uid']) && isset($_SESSION['role']) && $_SESSION['role'] === 'customer') {
            $wl_items = wishlist_get($this->conn, (int)$_SESSION['uid']);
    ...
    public function product() {
        ...
        $reviews    = review_get_by_product($this->conn, $product_id);
        $avg_rating = product_get_avg_rating($this->conn, $product_id);
        ...
        if (isset($_SESSION['uid']) && isset($_SESSION['role']) && $_SESSION['role'] === 'customer') {
            $uid         = (int)$_SESSION['uid'];
            $in_wishlist = wishlist_is_in($this->conn, $uid, $product_id);
            $can_review  = review_can_review($this->conn, $uid, $product_id);
        }
    ...
    public function cart() {
        ...
        $cart_items = cart_get($this->conn);
        $subtotal   = cart_subtotal($this->conn);
    ...
    public function checkout() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($shipping_address_id > 0) {
                $addr_row = address_get_by_id($this->conn, $shipping_address_id);
            ...
            if ($coupon_code !== '' && empty($errors)) {
                ...
                $coupon_result   = coupon_validate($this->conn, $coupon_code, $first_seller_id, $subtotal);
            ...
            if (empty($errors)) {
                ...
                $zone         = zone_get_by_id($this->conn, $zone_id);
                ...
                $order_id = order_create($this->conn, $order_data);
                ...
                if ($order_id) {
                    ...
                    order_add_items($this->conn, $order_id, $items_to_add);

                    if ($coupon_id) {
                        coupon_increment_use($this->conn, $coupon_id);
                    }
    ...
    public function orderDetail() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($action === 'cancel') {
                if (order_can_cancel($this->conn, $order_id)) {
                    order_update_status($this->conn, $order_id, 'cancelled');
    ...
    public function wishlist() {
        ...
        $uid      = (int)$_SESSION['uid'];
        $wishlist = wishlist_get($this->conn, $uid);
    ...
    public function disputes() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($errors)) {
                dispute_create($this->conn, array(
                    'customer_id' => $uid,
                    'seller_id'   => $seller_id,
                    'order_id'    => $order_id,
