<?php
// ============================================================
// app/controllers/Seller_controller/SellerController.php
// Procedural controller — no classes (per CONTRIBUTING.md rule).
// Every action is a function named "seller_<action>" so the router
// dispatches ?c=seller&a=dashboard → seller_dashboard($conn).
// $conn is passed in by the router; never via $this.
//
// Auth gate: most seller actions require the seller to be both
// logged in AND approved by an admin. Use seller_require_auth()
// instead of require_role('seller') alone.
// ============================================================

// ----------------------------------------------------------------
// Auth gate. Called as the first line of every seller_* function
// that needs the seller to be both logged in AND approved.
// ----------------------------------------------------------------
function seller_require_auth($conn) {
    require_seller_approved($conn);
}

// ----------------------------------------------------------------
// dashboard()
// ----------------------------------------------------------------
function seller_dashboard($conn) {
    seller_require_auth($conn);

    $sid    = (int)$_SESSION['sid'];
    $seller = seller_get_by_id($conn, $sid);

    // Count pending order items for this seller
    $all_seller_orders = order_get_by_seller($conn, $sid);
    $pending_count     = 0;
    ...
    $low_stock     = product_get_low_stock($conn, $sid, 5);
    $earnings      = analytics_earnings($conn, $sid, 'day');
    $all_returns   = return_get_by_seller($conn, $sid);
    $return_count  = 0;
    foreach ($all_returns as $r) {
        if ($r['status'] === 'pending') $return_count++;
    }
    $all_reviews  = review_get_by_seller($conn, $sid);
    $review_count = 0;
    foreach ($all_reviews as $rv) {
        if (empty($rv['seller_reply'])) $review_count++;
    }
    $all_disputes   = dispute_get_by_seller($conn, $sid);
    $dispute_count  = 0;
...
function seller_products($conn) {
    ...
    if ($do === 'toggle' && $pid > 0) {
        $p = product_get_by_id($conn, $pid);
        if ($p && (int)$p['seller_id'] === $sid) {
            product_toggle_availability($conn, $pid);
        }
    ...
    if ($do === 'delete' && $pid > 0) {
        $p = product_get_by_id($conn, $pid);
        if ($p && (int)$p['seller_id'] === $sid) {
            if (!product_has_pending_orders($conn, $pid)) {
                product_delete_images($conn, $pid);
                product_delete($conn, $pid);
    ...
    $products      = product_get_by_seller($conn, $sid);
    $low_stock_raw = product_get_low_stock($conn, $sid, 5);
    $low_stock_ids = array();
...
function seller_addProduct($conn) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($errors)) {
            $product_id = product_create($conn, array(
                'seller_id'          => $sid,
                ...
                if (isset($_FILES['images']) && is_array($_FILES['images']['error'])) {
                    ...
                    for ($i = 0; $i < min(4, count($extra_files['error'])); $i++) {
                        ...
                        if (move_uploaded_file($extra_files['tmp_name'][$i], $dest2)) {
                            product_add_image($conn, $product_id, 'product_images/' . $fname2, $order);
                            $order++;
                        }
                    }
                }
...
// editProduct()
// ----------------------------------------------------------------
function seller_editProduct($conn) {
    seller_require_auth($conn);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($errors)) {
            product_update($conn, $product_id, array(
                'category_id'        => $category_id,
                'name'               => $name,
                'description'        => $description,
                'price'              => (float)$price,
            ...
            if (isset($_FILES['images']) && is_array($_FILES['images']['error'])) {
                ...
                if ($has_new_extras) {
                    product_delete_images($conn, $product_id);
...
// orders()
// ----------------------------------------------------------------
function seller_orders($conn) {
    seller_require_auth($conn);

    $sid        = (int)$_SESSION['sid'];
    $status     = isset($_GET['status']) ? trim($_GET['status']) : 'all';
    $all_orders = order_get_by_seller($conn, $sid);
...
// orderDetail()
// ----------------------------------------------------------------
function seller_orderDetail($conn) {
    seller_require_auth($conn);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'confirm_item' && $item_id > 0) {
            order_item_update_status($conn, $item_id, 'confirmed', '');
            ...
            order_item_update_status($conn, $item_id, 'shipped', $tracking_note);
            set_flash('success', 'Item marked as shipped.');
        }
        redirect(BASE_URL . '?c=seller&a=orderDetail&id=' . $order_id);
    }

    $items = order_get_items_by_seller($conn, $order_id, $sid);
...
function seller_coupons($conn) {
    ...
    $coupons = coupon_get_by_seller($conn, $sid);
...
// analytics()
// ----------------------------------------------------------------
function seller_analytics($conn) {
    seller_require_auth($conn);

    $sid = (int)$_SESSION['sid'];

    $revenue      = analytics_revenue($conn, $sid, 'month');
    $top_products = analytics_top_products($conn, $sid, 5);
    $order_volume = analytics_order_volume($conn, $sid, 'month');
    $earnings     = analytics_earnings($conn, $sid, 'month');
...
// reviews()
// ----------------------------------------------------------------
function seller_reviews($conn) {
    seller_require_auth($conn);

    $sid     = (int)$_SESSION['sid'];
    $reviews = review_get_by_seller($conn, $sid);
...
function seller_returns($conn) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (in_array($action, array('approve', 'reject')) && $return_id > 0) {
            ...
            return_update_status($conn, $return_id, $new_status, $sid);
    ...
    $returns = return_get_by_seller($conn, $sid);
...
// disputes()
// ----------------------------------------------------------------
function seller_disputes($conn) {
    seller_require_auth($conn);
    $sid      = (int)$_SESSION['sid'];
    $disputes = dispute_get_by_seller($conn, $sid);
...
// disputeDetail()
// ----------------------------------------------------------------
function seller_disputeDetail($conn) {
    seller_require_auth($conn);
    $sid    = (int)$_SESSION['sid'];
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($errors)) {
            $ok = dispute_seller_respond($conn, $id, $sid, $response, $action);
            if ($ok) {
    ...
    $dispute = dispute_get_one_for_seller($conn, $id, $sid);
    if (!$dispute) {
